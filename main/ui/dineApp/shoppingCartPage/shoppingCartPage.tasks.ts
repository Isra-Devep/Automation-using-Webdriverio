...
export async function openOrderSummary(priceOfItem1,firstMerchandise,quantityFirstMerchandise,priceOfItem2,secondMerchandise,quantitySecondMerchandise)
{
    await (locators.tipsBtn.click());
    await (locators.tipsBtn.setValue(credentials.Store.tipGiven));
    await browser.pause(3000);

    let subTotalCalculated=Math.round(Number((priceOfItem1*quantityFirstMerchandise)+(priceOfItem2*quantitySecondMerchandise))*100)/100;
    console.log("subtotal calculated is "+ subTotalCalculated);
    let actualSubTotalPromise=(await locators.subTotal.getText());
    let actualSubTotal=Math.round(Number(actualSubTotalPromise.toString().substring(actualSubTotalPromise.indexOf('$')+1))*100)/100;
    console.log("subtotal actual is "+ actualSubTotal);
    await assertions.verifyElementsMatches(actualSubTotal,subTotalCalculated,"subtotals dont match");
    ...
    let totalbBillCalculated=await Math.round(await (subTotalCalculated+(credentials.Store.tipGiven)+(Number(taxAmount.substring(taxAmount.indexOf('$')+1))))*100)/100
    console.log("total calculated is "+ totalbBillCalculated);
    let actualTotalBillPromise=(await (commonActions.getAttribute(locators.totalPrice,"test-data")));
    ...
    assertions.verifyElementsMatches(Math.round(actualTotalBill*100)/100,Number(totalbBillCalculated),"totals dont match");
