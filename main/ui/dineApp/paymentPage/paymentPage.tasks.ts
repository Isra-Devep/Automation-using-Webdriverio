...
export async function addPaymentInformation() {
  await commonActions.sendKeysToElement(
    locators.phoneNumberField,
    credentials.paymentMethodInfo.creditCardPhoneNumber
  );
  await commonActions.sendKeysToElement(
    locators.nameField,
    credentials.paymentMethodInfo.creditCardName
  );

  await browser.switchToFrame(await $(locators.iframeForCreditCard));

  await locators.cardNumberField.setValue(
    credentials.paymentMethodInfo.creditCardNumber
  );

  await browser.switchToParentFrame();
  await commonActions.click(locators.phoneNumberCheckBox);
  const el = await locators.continueBtn;
  await el.click();
