...
Then(
  "User locates the store they wish to purchase from and opens it",
  async () => {
...
Then(
  "User confirms that the store is open and order can be placed for pickup",
  async () => {
    //skiped for now
...
Then(
  "User locates the merchandise they want to purchase and confirms that they are available",
  async () => {
    await menuPageActions.checkMerchandiseAvailability(
      credentials.Store.firstMerchandise
    );
  }
);

Then(
  "User adds the items to the cart and confirms that the cart is updated",
  async () => {
    await console.log("we're here");
    priceOfItem1 = await menuPageActions.addMerchandiseToCart(
      credentials.Store.firstMerchandise,
      credentials.Store.quantityFirstMerchandise
    );
...
Then(
  "Cart is updated then the user adds more items to the cart and verifies the cart again",
  async () => {
    priceOfItem2 = await menuPageActions.addMerchandiseToCart(
      credentials.Store.secondMerchandise,
      credentials.Store.quantitySecondMerchandise
    );
  }
);

Then(
  "User clicks on the cart button to, adds a tip, verifies total bill and continues to payment",
  async () => {
    await menuPageActions.openCart();
    browser.pause(3000);
    totalBill = await shoppingCartPageActions.openOrderSummary(
      priceOfItem1,
      credentials.Store.firstMerchandise,
      credentials.Store.quantityFirstMerchandise,
      priceOfItem2,
...
When(
  "The payment is processed the user confirms the information is correct",
  async () => {
